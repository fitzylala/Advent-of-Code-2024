...
public class Day5 {
    public static void main(String[] args) {
        ...
        ArrayList<ArrayList<Integer>> orderRules = new ArrayList<ArrayList<Integer>>();
        for (String s : inputSplit) {
            ArrayList<Integer> rule = new ArrayList<Integer>();
            rule.add( Integer.valueOf(s.substring(0,2)) );
            rule.add( Integer.valueOf(s.substring(3)) );
            orderRules.add(rule);
        }

        String input2 = "46,98,96,88,42\n" +
        ...
        String[] input2Split = input2.split("\\R+");
        ArrayList<ArrayList<Integer>> prints = new ArrayList<ArrayList<Integer>>();
        for (String line : input2Split) {
        ...
        int total = 0;
        int fixedTotal = 0;
        for (ArrayList<Integer> print : prints) {
            boolean valid = true;
            for (ArrayList<Integer> rule : orderRules) {
                if (print.contains(rule.get(0)) && print.contains(rule.get(1))) {
                    if (print.indexOf(rule.get(0)) > print.indexOf(rule.get(1))) {
                        valid = false;
                    }
                }
            }
            if (valid) {
                int midpoint = (print.size() - 1)/2;
                total += print.get(midpoint);
            }
                ...
                for (int i=0; i<print.size(); i++) {
                    for (ArrayList<Integer> rule : orderRules) {
                        if (print.contains(rule.get(0)) && print.contains(rule.get(1))) {
                            if (print.indexOf(rule.get(0)) > print.indexOf(rule.get(1))) {
                                Collections.swap(print, print.indexOf(rule.get(0)), print.indexOf(rule.get(1)));
                            }
                        }
                    }
                }
                int midpoint = (print.size() - 1)/2;
                fixedTotal += print.get(midpoint);
            }


        }
        System.out.println("Part 1 total: " + total);
        System.out.println("Part 2 total: " + fixedTotal);
